...
gulp.task('gcs-publish-pdf', () => {
  return gulp
    .src('src/pdf/*.pdf')
    .pipe(gzip()) // optional
    .pipe(gcPub({
      bucket: 'rivendellweb-bucket-data',
      keyFilename: 'rivendellweb-data-storage-ca7e22972b0a.json',
      projectId: 'rivendellweb-data-storage',
      base: '/pdf', // => File will be uploaded to rivendellweb-bucket-data/pdf
      public: true
    }));
});
 ...
 */
gulp.task('pdf-build', () => {
  runSequence('markdown', 'build-pm-template', 'build-pdf', 'copy-pdf');
});
